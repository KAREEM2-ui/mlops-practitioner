...
def test_postprocess_returns_empty_without_qualifying_detections(
    detections, get_original_size
):
...
def test_single_detection_scales_box_and_crops_mask(get_original_size):
    ...
    predictions = Pipeline.postprocess(
        make_outputs([[320, 320, 320, 320, 0.9]]), original_size
    )
    assert len(predictions) == 1
    prediction = predictions[0]
    assert prediction.confidence == pytest.approx(0.9)
    assert prediction.box.model_dump() == dict(
        x_min=original_size[0] // 4,
        y_min=original_size[1] // 4,
        x_max=original_size[0] * 3 / 4,
        y_max=original_size[1] * 3 / 4,
    )
